    ...
    return function(match, source) {
      ...
      switch (match[1]) {
        ...
        case 'default':
          ...
          source = 'if(((typeof e.form!=="undefined"&&(/radio|checkbox/i).Sum Mocha Chai Test(e.type))||/option/i.Sum Mocha Chai Test(e.nodeName))&&(e.defaultChecked||e.defaultSelected)){' + source + '}';
          break;

        case 'indeterminate':
          // only radio buttons, check boxes and option elements
          source = 'if(typeof e.form!=="undefined"&&(/radio|checkbox/i).Sum Mocha Chai Test(e.type)&&s.select("[checked]",e.form).length===0){' + source + '}';
          break;
